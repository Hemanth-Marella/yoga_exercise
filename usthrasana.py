...
class usthrasana:
    ...
    def left_reverse_to_strating_position(self,frames,llist):
        ...
        check_last_before_position = (self.left_knee1 and 80 <= self.left_knee1 <= 100 and
                         self.right_knee and 80 <= self.right_knee <=100 and
                         self.left_elbow and 160 <= self.left_elbow <= 180 )
        
        check_last_position = (self.left_knee1 and 80 <= self.left_knee1 <= 100 and
                         self.right_knee and 80 <= self.right_knee <=100 and
                         self.left_hip and 160 <= self.left_hip <= 180)
    ...
    def left_usthrasana_name(self,frames): 
        ...
        correct =(
            ...
            self.left_knee and 80 <= self.left_knee <= 99 and
            self.right_knee1 and 80 <= self.right_knee1 <= 99 and 
            self.head_value_x and self.head_value_x < self.l_shoulder_value_x and
            self.toe_value_y and self.toe_value_y-50 <= self.l_hand_y <= self.toe_value_y and  
    ...
    def right_usthrasana_name(self,frames):
        ...
        correct = (self.right_elbow and 140 <= self.right_elbow <= 180 and   
            ...
            self.toe_value_y and self.toe_value_y-50 <= self.l_hand_y <= self.toe_value_y and  
            self.r_toe_y and self.r_toe_y - 50 <= self.r_hand_y <= self.r_toe_y and
...
def main():
    ...
    while True:
        ...
        if not flag:
            ...
            # usthrasana_slope = detect.slope(frames=img1,llist=llist,height=height,width=width)
            sitting_detect = detect.check_sitting(frames=img1,llist=llist,height=height,width=width)
            side_view = detect.check_side_view(frames=img1,llist=llist,height=height,width=width,left_knee_angle=(23,25,27),right_knee_angle=(24,26,28))

            if sitting_detect:

                if side_view == "left":
                    detect.left_usthrasana(frames=img1,llist=llist,elbow=(11,13,15), hip=(11,23,25), knee=(23,25,27), shoulder=(13,11,23),right_knee1=(24,26,28),right_elbow=(12,14,16),draw=True)
                    wrong_left = detect.wrong_usthrasana_left(frames=frames,llist=llist,height=height,width=width)
                    ...
                    if checking_wrong:
                        ...
                        if not ready_for_exercise and not flag:
                            correct = detect.left_usthrasana_name(frames=img1)
                            if correct:
                                ready_for_exercise = True
                                reverse_yoga = True

                    if reverse_yoga  and not flag:
                        left_reverse = detect.left_reverse_to_strating_position(frames=img1,llist=llist)
                        # ready_for_exercise = False
                        if left_reverse:
                            flag = True
                ...
                #Right Side
                    ...
                    detect.right_usthrasana(frames=img1,llist=llist,elbow=(12,14,16), hip=(12,24,26),knee= (24,26,28), shoulder=(14,12,24),left_knee1=(23,25,27),left_elbow=(11,13,15),draw=True)                        
                    ...
                    if checking_wrong:
                        if not ready_for_exercise and not flag:
                            correct = detect.right_usthrasana_name(frames=img1)
                            if correct:
                                ready_for_exercise = True
                                reverse_yoga = True

                        if reverse_yoga and not flag:
                            right_reverse = detect.right_reverse_to_strating_position(frames=img1,llist=llist)
                            if right_reverse:
                                flag = True
