...
# import exercise_data.exercise_CRUD as exercise_CRUD
# import cv2
# from headPose import HeadPoseEstimator
# from utility_files.draw_overlay import draw_countdown_circle_on_image


# class Gomukhasana:
    
#     def _init(self, liveFeed, mainObject, exer_data, poseDetector, TIME_GAP_BETWEEN_VOICE = 1, INDIVIDUAL_POSTUREOR_MERGED_POSTURE = 1):
#         exercise_CRUD.set_data(exer_data)
         
#         self.liveFeed = liveFeed
#         self.mainObject = mainObject
#         self.detector = poseDetector
#         self.TIME_GAP_BETWEEN_VOICE = TIME_GAP_BETWEEN_VOICE
        
#         self.pose = poseDetector.pose
        
#         self.audioPlayer = mainObject.audioPlayer
        
#         print(INDIVIDUAL_POSTURE_OR_MERGED_POSTURE)
#         self.INDIVIDUAL_POSTURE = 1
#         self.MERGED_POSTURE = -1
#         self.INDIVIDUAL_POSTURE_ORMERGED_POSTURE = INDIVIDUAL_POSTUREOR_MERGED_POSTURE
        
#         self.repetitions = 0
#         self.sets = 0
#         self.minRepetitions = exer_data['repetitions']['min_repetitions']  # Minimum Repetitions For 1-Set
#         self.maxSets = exer_data['sets']['maxSets'] # Maximum Sets For WorkOut Completion
#         self.isWorkOutCompleted = False
#         self.setCompleted  = False
#         self.checkTime_for_holdingBody = 0
#         self.OPTN_REP_COUNT, self.OPTN_SET_COMPLETION_ANNOUNCEMENT, self.OPTN_REST_ANNOUNCEMENT, self.OPTN_NEXT_SET_ANNOUNCEMENT, self.OPTN_EXERCISE_COMPLETION_ANNOUNCEMENT, self.OPTN_NOTHING_TO_DO_AFTER_WORKOUT_COMPLETION = 0, 1, 2, 3, 4, -1
...
#         self.m_lmlist = []
#         self.angle =0
#         self.all_methods = liveFeed.allMethods
#         self.head_pose = HeadPoseEstimator(mainObject=mainObject)
#         # self.stop_ = False
...
#         imgRB = cv2.cvtColor(frames,cv2.COLOR_BGR2RGB)
#         self.results = self.pose.process(imgRB)
...
#                 if draw:
#                     cv2.circle(frames,(px,py),5,(255,255,0),2)
...
#             cv2.putText(frames,f'l_elbow{str(self.left_elbow)}',(10,40),cv2.FONT_HERSHEY_PLAIN,2,(0,0,0),2)
#             cv2.putText(frames,f'l_hip{str(self.left_hip)}',(10,80),cv2.FONT_HERSHEY_PLAIN,2,(0,0,0),2)
#             cv2.putText(frames,f'l_knee{str(self.left_knee)}',(10,120),cv2.FONT_HERSHEY_PLAIN,2,(0,0,0),2)
#             cv2.putText(frames,f'l_shoulder{str(self.left_shoulder)}',(10,160),cv2.FONT_HERSHEY_PLAIN,2,(0,0,0),2)

#             cv2.putText(frames,f'r_elbow{str(self.right_elbow)}',(10,40),cv2.FONT_HERSHEY_PLAIN,2,(0,0,0),2)
#             cv2.putText(frames,f'r_hip{str(self.right_hip)}',(10,80),cv2.FONT_HERSHEY_PLAIN,2,(0,0,0),2)
#             cv2.putText(frames,f'r_knee{str(self.right_knee)}',(10,120),cv2.FONT_HERSHEY_PLAIN,2,(0,0,0),2)
#             cv2.putText(frames,f'r_shoulder{str(self.right_shoulder)}',(10,160),cv2.FONT_HERSHEY_PLAIN,2,(0,0,0),2)
...
#             self.all_methods.reset_after_40_sec()
#             self.all_methods.play_after_40_sec(["you completed your yoga pose corrected"],llist=llist)
#             self.checkTime_for_holdingBody+=1
#             draw_countdown_circle_on_image(img=frames, countdown_time=self.checkTime_for_holdingBody//10)
#             # cv2.putText(frames,str("Vajrasana"),(40,50),cv2.FONT_HERSHEY_DUPLEX,2,(0,0,255),2)
#             if self.checkTime_for_holdingBody//10 >= 3:
#                 return True
#         return False
#     def reverse_gomukhasana(self,frames,llist):
...
#     def voiceCompletedCB(self, opId):
#         # print(f'OPERETION ID................{opId}')
#         match opId:
#             case self.OPTN_REP_COUNT:
#                 if self.repetitions==self.minRepetitions:
#                     self.sets += 1
#                     exercise_CRUD.current_set = self.sets
#                     exercise_CRUD.update_set()
#                     print(f'SET NO:=> {self.sets}')
                             
                   
#                     self.repetitions = 0
#                     self.taken_position = False
#                     self.setCompleted = True
#                     # self.reset_voiceHoldingTime_AND_oneTimeRunFlag(oneTimeRunFlag="oneTimeRun_flag_for_resetAll", ignoringFlag="RESET_ALL_FLAGS")
                    
#                     self.audioPlayer.playAudio([""],
#                                                self.voiceCompletedCB, 
#                                                 self.OPTN_SET_COMPLETION_ANNOUNCEMENT 
#                                                 )
#                 else:
#                     self.setCompleted = False
                    
                            
#             case self.OPTN_SET_COMPLETION_ANNOUNCEMENT:
#                 if self.sets == self.maxSets:
#                     self.isWorkOutCompleted = True
#                     self.setCompleted = False
#                     self.audioPlayer.playAudio([""],
#                                                self.voiceCompletedCB, 
#                                                 self.OPTN_EXERCISE_COMPLETION_ANNOUNCEMENT 
#                                                 )
#                 else:
#                     self.liveFeed.announce_Yoga_Comletion_Without_Sets_And_Repetitions(self.voiceCompletedCB,
#                                                                                             "Cow Face Pose",
#                                                                                             self.OPTN_REP_COUNT,
#                                                                                             True,
#                                                                                             True)  
                            
#             case self.OPTN_EXERCISE_COMPLETION_ANNOUNCEMENT:
#                 self.setCompleted = False
#                 self.liveFeed.announce_Exercise_Completion(self.sets)
            
#             case self.OPTN_NOTHING_TO_DO_AFTER_WORKOUT_COMPLETION:
#                 self.setCompleted = False
...
#             self.pose_positions(frames,draw=False)
#             llist = self.pose_landmarks(frames,False)
...
#                 self.gomukhasana(frames=frames,llist=llist,left_elbow=(11,13,15), left_hip=(11,23,25), left_knee=(23,25,27), left_shoulder=(13,11,23),right_elbow=(12,14,16), right_hip=(12,24,26),right_knee= (24,26,28), right_shoulder=(14,12,24),draw=False)
#                 self.check_slopes(frames=frames,lmlist=llist,height=height,width=width)

#                 if not check_wrong:
                
#                     wrong = self.wrong_gomukhasana(frames=frames,llist=llist,height=height,width=width)
#                     if wrong:
#                         check_wrong = True

#                 if check_wrong and not check_position:
#                     correct = self.gomukhasana_name(frames=frames)
#                     if correct:
#                         check_position = True
                        
#                         self.repetitions +=1
#                         exercise_CRUD.current_rep = self.repetitions
#                         exercise_CRUD.rep_list.append(exercise_CRUD.current_accuracy)
#                         exercise_CRUD.update_rep()
#                         print(f"GOMUKHASANA : {self.repetitions}")
#                         self.checkTime_for_holdingBody = 0

#                 if check_position:
                    
#                     reverse = self.reverse_gomukhasana(frames=frames,llist=llist)
#                     if reverse:
#                         return True
        
    
    
#     # REFRESHING SETS AND REPETITIONS BASED ON REQUIREMENTS: 
#     def refresh_Set_Repetition(self):
#         self.repetitions = 0
#         self.sets = 0
#         self.setCompleted = False
#         self.checkTime_for_holdingBody = 0
...
# #     video_capture.set(cv2.CAP_PROP_FRAME_WIDTH, 1480)  #width
# #     video_capture.set(cv2.CAP_PROP_FRAME_HEIGHT, 980) 
# #     global  llist
# #     detect = Gomukhasana()
# #     check_wrong = False
# #     check_position = False
# #     flag = False
# #     ref_video = cv2.VideoCapture("second videos/gomukhasana.mp4")
...
# #         # resized_img = cv2.resize(img, None, fx=3.0, fy=3.0, interpolation=cv2.INTER_LINEAR)

# #         # ret_ref, ref_frame = ref_video.read()
# #         # if not ret_ref:
# #         #     ref_video.set(cv2.CAP_PROP_POS_FRAMES, 0)  # loop video
# #         #     ret_ref, ref_frame = ref_video.read()

# #         # # Resize reference video frame and place it at top-left
# #         # if ret_ref:
# #         #     ref_frame = cv2.resize(ref_frame, (400, 300))
# #         #     h, w, _ = ref_frame.shape
# #         #     frames[0:h, 0:w] = ref_frame  # Overlay in corner
